...
class KAN_CLAM_MB_v4(CLAM_MB):
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, return_embedding=False):
        assert len(h.shape) == 3
        ...
        if self.training or return_embedding:
            # 获取mask
...
class KAN_CLAM_MB_v5(CLAM_MB):
    ...
    def forward(self, h, scores=None, only_attn=False):
        ...
        if scores is None:
            ...
            # 直接丢弃未被选中的实例，不参与计算（因此此时那些实例，对于引导LIPN蒸馏没有意义）
            select_logits_ = torch.empty(B, self.n_classes).float().to(device)
            for c in range(self.n_classes):
            ...
            # 直接丢弃未被选中的实例，不参与计算（因此此时那些实例，对于引导LIPN蒸馏没有意义）但是获得patch级特征的方式不同
            select_logits_ = torch.empty(B, self.n_classes).float().to(device)
            for c in range(self.n_classes):
...
class ImageProcessor(nn.Module):    # LIPN
...
class ImageProcessor_MoE(nn.Module):    # DB-LIPN
    def __init__(self, args):
        super().__init__()
        ...
        if self.args.lwc == 'mbv4t':
            self.extractors = nn.ModuleList([mobilenetv4_conv_tiny(n_classes=args.n_classes) for _ in range(args.num_moe)])
        else:
            raise NotImplementedError

    def forward(self, imgs, phase='infer'): 
        img = imgs.permute(0, 1, 4, 2, 3).float() / 255. - 0.5
        B, N, C, H, W = img.shape
        assert B == 1
        img = img.reshape(B * N, C, H, W)
        
        if phase == 'attn_train':
            scores = [self.extractors[i](img).reshape(B, N, self.args.n_classes) for i in range(len(self.extractors))]                     
            dic_scores = [_gumbel_sigmoid(scores[i], tau=0.7, hard=True, eps=1e-10, training=False, threshold=0.5) for i in range(len(self.extractors))]
            return scores, dic_scores
        elif phase == 'hard_train':
            scores = [self.extractors[i](img).reshape(B, N, self.args.n_classes) for i in range(len(self.extractors))]
            dic_scores = [_gumbel_sigmoid(scores[i], tau=0.7, hard=True, eps=1e-10, training=False, threshold=0.5) for i in range(len(self.extractors))]
            return dic_scores
        elif phase == 'infer': 
            scores = torch.cat([self.extractors[i](img).reshape(B, N, self.args.n_classes) for i in range(len(self.extractors))], 0)
            dic_scores = _gumbel_sigmoid(torch.mean(scores, 0, keepdim=True), tau=0.7, hard=True, eps=1e-10, training=False, threshold=0.5)
            return dic_scores
        

    def cross_EMA(self):
        if len(self.extractors) > 1:
            cached_params = [copy.deepcopy(list(expert.parameters())) for expert in self.extractors]

            for i, expert in enumerate(self.extractors):
                with torch.no_grad():
                    for param_idx, param in enumerate(expert.parameters()):

                        avg_param = sum(
                            cached_params[j][param_idx].data for j in range(self.args.num_moe) if j != i
                        ) / (self.args.num_moe - 1)
                        

                        param.data = (1 - self.args.LIPN_ema) * param.data + self.args.LIPN_ema * avg_param
